  ...
  return (
    <Card className="border-border bg-card">
      <CardHeader className="pb-3">
        <h3 className="text-sm font-medium text-foreground">Platform Comparison</h3>
      </CardHeader>
      <CardContent>
          ...
          <table className="w-full text-xs" role="table">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="pb-2 px-2 text-right font-medium text-chart-3">Kalshi</th>
                <th className="pb-2 pl-2 text-right font-medium text-chart-5">Polymarket</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.label} className="border-b border-border/50 last:border-0">
                  ...
                  <td className={cn(
                    ...
                    row.best === "kalshi" ? "text-primary" : "text-foreground"
                  )}>
                    {row.kalshi}
                    {row.best === "kalshi" && <span className="ml-1 text-primary">*</span>}
                  </td>
                  <td className={cn(
                    "py-2 pl-2 text-right font-mono font-medium",
                    row.best === "polymarket" ? "text-primary" : "text-foreground"
                  )}>
                    {row.poly}
                    {row.best === "polymarket" && <span className="ml-1 text-primary">*</span>}
                  </td>
                </tr>
              ))}
            </tbody>
