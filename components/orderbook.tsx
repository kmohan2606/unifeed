...
import type { Market, OrderbookData, OrderbookEntry, Platform } from "@/lib/types"
      ...
      {showPlatform && (
        <span
          className={cn(
            ...
            entry.platform === "kalshi" ? "text-chart-3" : "text-chart-5"
          )}
        >
    ...
    <div className="flex items-center justify-between border-y border-border bg-accent/30 px-3 py-1.5">
      ...
      <div className="flex items-center gap-2">
        ...
        <Badge
          ...
          className={cn(
            "text-[10px] font-mono",
            book.spreadPercent < 3 ? "text-success" : book.spreadPercent < 5 ? "text-warning" : "text-destructive"
          )}
        >
    ...
    <div className="flex items-center justify-between border-t border-border px-3 py-1.5">
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-muted-foreground">Bids: {formatQty(maxBidTotal)}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="h-2 w-2 rounded-sm bg-destructive" />
          <span className="text-[10px] text-muted-foreground">Asks: {formatQty(maxAskTotal)}</span>
        </div>
      </div>
    </div>
  )
...
function SingleBook({ title, platform, book, isExpanded, onToggle, showPlatformLabels = false, accentColor }: SingleBookProps) {
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-[10px] font-mono text-muted-foreground">
            <span>Mid: <span className="text-foreground font-medium">{formatPrice(book.midPrice)}{'\u00A2'}</span></span>
            <span>Spread: <span className={cn(
              "font-medium",
              book.spreadPercent < 3 ? "text-success" : book.spreadPercent < 5 ? "text-warning" : "text-destructive"
            )}>{book.spreadPercent.toFixed(2)}%</span></span>
          </div>
        ...
        <CardContent className="p-0">
          ...
          <div className="border-b border-border">
            {reversedAsks.map((entry, i) => (
              <OrderbookRow key={`ask-${i}`} entry={entry} maxTotal={maxAskTotal} side="ask" showPlatform={showPlatformLabels} />
            ))}
          </div>
          ...
          <div>
            {book.bids.map((entry, i) => (
              <OrderbookRow key={`bid-${i}`} entry={entry} maxTotal={maxBidTotal} side="bid" showPlatform={showPlatformLabels} />
            ))}
          </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-muted-foreground font-mono">Refreshes every 3s</span>
      </div>
