...
export function PortfolioView() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(apiUrl("/api/portfolio/positions"), { headers }).then((r) =>
        r.json(),
      ),
      fetch(apiUrl("/api/portfolio/balance"), { headers }).then((r) =>
        r.json(),
      ),
    ])
      .then(([pos, bal]) => {
            ...
            <CardContent>
              <div className="flex flex-col gap-3">
                {(["kalshi", "polymarket"] as const).map((platform) => {
                  const platformPositions = positions.filter(
                    (p) => p.platform === platform,
                  );
                  const platformValue = platformPositions.reduce(
                  ...
                  return (
                    <div
                      key={platform}
                      className="flex items-center justify-between rounded-md bg-secondary/50 p-2.5"
                    >
                      <div>
                        <span
                          className={cn(
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          {platformPositions.length} positions
                        </p>
                      </div>
                      <span className="text-sm font-semibold font-mono text-foreground">
                        {formatUSD(platformValue)}
                      </span>
                    </div>
                  );
                })}
